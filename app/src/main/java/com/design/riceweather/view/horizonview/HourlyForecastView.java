...
import com.design.riceweather.util.DisplayUtil;
import com.design.riceweather.util.IconUtils;
...
public class HourlyForecastView extends View implements ScrollWatcher {
    ...
    public void initData(List<WeatherHourlyBean.HourlyBean> weatherData) {
        Log.d("点位", "hourlyForecastView 视图加载数据");
        ...
        /*
          使整个视图无效。如果该视图可见，{@link #onDraw(android.graphics.Canvas)} 在将来的某个时候会被调用。
          <p>
          必须从UI线程调用此方法。 要从非UI线程进行调用，请调用{@link #postInvalidate()}.
         */
        invalidate();
    }
...
//        setLayerType(View.LAYER_TYPE_SOFTWARE, null);//关闭硬件加速
        ...
        paint1.setColor(mContext.getResources().getColor(R.color./*line_back_dark*/white60));
        ...
        foldLinePaint.setColor(mContext.getResources().getColor(R.color./*line_color*/white90));
        ...
        textLinePaint.setColor(mContext.getResources().getColor(R.color./*black*/white));
        textPaint.setColor(mContext.getResources().getColor(R.color./*search_light_un_color*/white54));
...
//            canvas.drawText(text, drawPoint, bitmapHeight + bitmap.getHeight() + 100 / 3f, textPaint);
    ...
    private int getScrollBarX() {
        Log.d(TAG, "点位： getScrollBarX: maxScrollOffset : " + maxScrollOffset);
        int x = (ITEM_SIZE - 1) * itemWidth * scrollOffset / (maxScrollOffset/* + 1*/);
